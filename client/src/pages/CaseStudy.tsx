...
export default function CaseStudy() {
  ...
  return <article className={`case-study case-study--${world.slug}`} style={{ "--case-accent": world.accent } as React.CSSProperties} data-cursor-color={world.accent}>
    <header className="case-nav"><Link href={`/portfolios/${world.slug}`}><ArrowLeft size={16} /> BACK TO {world.title.toUpperCase()}</Link><Link href="/" className="case-nav__brand"><img src="https://files.manuscdn.com/user_upload_by_module/session_file/310519663761893749/ZIngFRXDTRRZUcRn.png" alt="" /> SPARSH MISHRA <span>/ CONSTELLATION</span></Link><span>{world.coordinate}</span></header>
